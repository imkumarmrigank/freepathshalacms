...
const FOCUS = [
  { value: "written", label: "Only days meetings were written up" },
  { value: "backdated", label: "Only back-dated write-ups" },
  { value: "counselling", label: "Only days with counselling work" },
];
...
export default async function MentorDayBookPage({
  ...
  const rows = await mentorDays(from, to, centerId, who, focus, pg);

  // which of these days the person wrote up, for the column
  const written = await staffNoteDays(from, to, [...new Set(rows.map((r) => r.mentor_id))]);
            ...
            <table className="tbl">
              <thead>
                <tr>
                  ...
                  <th>Referrals</th><th>Counselling steps</th><th>Feedback</th><th>Wrote up</th>
                </tr>
              </thead>
              <MentorRows rows={rows} centerId={centerId} notes={notes} />
            </table>
          </div>
        )}
        <Pager page={pg.page} pages={win.pages} first={win.first} last={win.last}
          total={total} unit="day of mentor work" />
      </Card>
    </>
  );
}
