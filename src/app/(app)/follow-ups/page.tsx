...
export default async function FollowUpsPage({
  ...
  const rows = await query<{
    ...
    `SELECT i.id, s.first_name, s.last_name, s.enrollment_no, i.follow_up_date,
      ...
      ORDER BY i.follow_up_date NULLS LAST LIMIT 300`,
    params,
  );

  const overdue = rows.filter((r) => r.follow_up_date && r.follow_up_date.slice(0, 10) < today());
  const dueSoon = rows.filter((r) => {
    if (!r.follow_up_date) return false;
    const d = r.follow_up_date.slice(0, 10);
    const in7 = new Date(); in7.setDate(in7.getDate() + 7);
    return d >= today() && d <= in7.toISOString().slice(0, 10);
  });
  ...
  return (
    <>
      ...
      <div className="mb-5 grid gap-4 sm:grid-cols-3">
        <StatCard label="Overdue" value={overdue.length} hint="past the promised date"
          tone={overdue.length ? "bad" : "default"} />
        <StatCard label="Due this week" value={dueSoon.length} hint="next 7 days"
          tone={dueSoon.length ? "warn" : "default"} />
        <StatCard label="Open in total" value={rows.length} hint="still pending" />
      </div>
